...
(function(b){"function" === typeof define && define.amd?define(["jquery"], b):"object" === typeof exports?module.exports = b(require("jquery")):b(jQuery)})(function(b){b.fn.jR3DCarousel = function(E){function x(){q(); v = setInterval(A, a.animationInterval + a.animationDuration)}function q(){v && clearInterval(v)}function A(){r = u + 1; y.run(a.animation, r); z()}function B(F){r = Math.floor(u / c) * c + F; y.run(a.animation, r); z()}function w(){this.animations = {slide:G, slide3D:H, scroll:I, scroll3D:J, fade:K}}function G(a){d.css({perspective:"",
        ...
        l.add(m).css({position:"absolute", top:"50%", zIndex:1, transform:"translateY(-50%)", fontSize:"4em", color:"rgba(0, 0, 0, 0.97)", cursor:"pointer", userSelect:"none"}).appendTo(d).hide(); l.on("click", function(){r = u - 1; y.run(a.animation, r); z()}); m.on("click", function(){A()}); d.on("mouseenter touchstart", function(){l.add(m).show()}).on("mouseleave touchcancel", function(){l.add(m).hide()}); l.hover(function(){l.css({color:"rgba(201, 201, 201, 0.97)"})}, function(){l.css({color:"rgba(0, 0, 0, 0.97)"})});
        m.hover(function(){m.css({color:"rgba(201, 201, 201, 0.97)"})}, function(){m.css({color:"rgba(0, 0, 0, 0.97)"})}); b(document).on("keydown", function(c){var b = d[0].getBoundingClientRect(); (b = 0 < b.bottom && 0 < b.right && b.left < (innerWidth || document.documentElement.clientWidth) && b.top < (innerHeight || document.documentElement.clientHeight)) && 37 == c.which?(q(), "rtl" == a.rotationDirection?l.click():m.click()):b && 39 == c.which && (q(), "rtl" == a.rotationDirection?m.click():l.click())}); M(d, function(b){q(); "left" ==
        b?m.click():"right" == b?l.click(): - 1 != a.animation.indexOf("scroll") && ("down" == b?m.click():"up" == b && l.click())})}function v(){for (var e = b("<div class=navigation />").css({position:"absolute", bottom:0, right:0}), g = 0; g < c; g++)e.append("<div class=nav></div>"); "circles" == a.navigation && e.find(".nav").css({borderRadius:"12px"}); e.find(".nav").css({display:"none", margin:"5px", cursor:"pointer", backgroundColor:"rgba(255, 255, 255, 0.77)", width:"12px", height:"12px", transition:"all " + a.animationDuration +
